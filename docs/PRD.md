...
本项目为其官方 **Obsidian 插件**，把用户在产品里的阅读资产（文章、高亮、评论、对话）同步进 Obsidian 知识库，并在 Obsidian 内提供 AI 对话与 AI 续写能力。

## 2. 产品组成（两部分）

1. **Obsidian 插件**（本仓库主体）：同步引擎 + 聊天侧边栏 + 续写命令 + 设置页。
...
- 场景 3：在 Obsidian 里写作，让 AI 基于上文续写。
...
- 产品侧无写作模块；写作使用 Obsidian 自带编辑器，插件提供 **AI 续写**：
  - 编辑器命令「AI 续写」（命令面板 + 编辑器右键菜单）：取光标前文本（超限截断），发给产品对话后端，流式将续写内容插入光标处。
  - 续写过程可中断；插入内容不带任何标记污染。
...
5. 在任意笔记中执行「AI 续写」，光标处流式插入续写内容，可中断。
